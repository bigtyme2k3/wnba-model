...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "cross_market": load_json("data/dashboard/wnba_cross_market_top_plays.json", {}),
        "unified": load_json("data/dashboard/wnba_unified_player_simulation_v2.json", {}),
    }
...
function activeRecommendations(){let rows=A(DATA.cross_market?.portfolio);if(rows.length)return rows;return A(DATA.cross_market?.top_plays).filter(r=>['BET','LEAN'].includes(String(r.decision||r.action||'').toUpperCase())&&Number(r.recommended_units_final??r.recommended_units||0)>0)}
function activeUnits(){return activeRecommendations().reduce((s,r)=>s+(Number(r.recommended_units_final??r.recommended_units)||0),0)}
function kpis(){let s=summary(),active=activeRecommendations().length,units=activeUnits();return `<div class="grid"><div class="card"><div class="label mono">Odds</div><div class="big mono">${s.sportsbook_markets?'Loaded':'Missing'}</div><div class="small mono">${S(s.sportsbook_markets,0)} markets</div></div><div class="card"><div class="label mono">Props</div><div class="big mono">${S(s.props,0)}</div><div class="small mono">player prop rows</div></div><div class="card"><div class="label mono">Games Today</div><div class="big mono">${A(DATA.today_games).length}</div><div class="small mono">scheduled matchups</div></div><div class="card"><div class="label mono">Players</div><div class="big mono">${S(s.players,0)}</div><div class="small mono">tracked players</div></div><div class="card"><div class="label mono">Active Recommendations</div><div class="big mono">${active}</div><div class="small mono">BET/LEAN plays · ${units.toFixed(2)} units</div></div></div>`}
...
def apply_persistent_dashboard_patches() -> None:
    ...
    if missing:
        raise RuntimeError(f"Dashboard rebuild removed required blocks: {missing}")
...
def main() -> None:
    ...
    print("Dashboard V4 built with source-level Games KPI stabilization")
