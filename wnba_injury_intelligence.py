...
def player_pool():
 raw=load_json(RAW/'wnba_players_live.json',{});rows=[]
 if isinstance(raw,dict):
  for name,item in raw.items():
   if isinstance(item,dict):r=dict(item);r.setdefault('player',name);rows.append(r)
...
def todays_teams(master,target_date):
 teams=set();games=master.get('games',[]) or master.get('today_games',[]) or []
 for g in games:
  game_date=str(g.get('game_date') or master.get('target_date') or '')[:10];bucket=str(g.get('bucket') or 'today')
...
def guard_rows(rows,adjustments):
 ...
 for r in rows or []:
  ...
  if adj:
   ...
   row['projected_minutes_pre_injury']=row.get('projected_minutes_pre_injury',row.get('projected_minutes'))
   row['projected_minutes']=adj['projected_minutes'];row['minutes_delta']=adj['minutes_delta'];row['usage_delta']=adj.get('usage_delta',0);row['injury_projection_factor']=adj['projection_factor']
   projection_key=next((k for k in ('projection','proj','model_projection','projected_value') if row.get(k) not in (None,'')),None)
   if projection_key:
    before=sf(row.get(projection_key));row['projection_pre_injury']=row.get('projection_pre_injury',before)
    after=round(before*adj['projection_factor'],2);row[projection_key]=after;row['projection']=after;row['proj']=after;row['injury_projection_delta']=round(after-before,2)
   conf=sf(row.get('confidence',row.get('final_score')),0);row['confidence_pre_injury']=row.get('confidence_pre_injury',conf)
   adjusted_conf=max(0,round(conf-adj['confidence_penalty'],1));row['confidence']=adjusted_conf;row['final_score']=adjusted_conf;row['injury_confidence_delta']=round(adjusted_conf-conf,1)
   if adj['is_out']:
...
def build(target_date:str):
 master=next((load_json(p,{}) for p in MASTER_PATHS if p.exists()),{});teams=todays_teams(master,target_date);players=player_pool();injuries=read_csv(RAW/f'injuries_{target_date}.csv') or read_csv(RAW/'injuries_today.csv')
 filtered=[]
 for r in injuries:
 ...
 injury_by={r['player_key']:r for r in filtered if r['player_key']};roster=defaultdict(list)
 ...
 for team in teams or roster.keys():
  ...
  for inj in team_inj:
   p=players.get(inj['player_key'],{});base=sf(p.get('mpg'),25);usage=sf(p.get('usage'),18);factor=STATUS_FACTORS[inj['severity']];proj=round(base*factor,1)
  ...
  candidates=[p for p in roster.get(team,[]) if norm(p['player']) not in injury_by and p['mpg']>0];candidates.sort(key=lambda p:p['mpg'],reverse=True);weights=[]
 ...
 for path in MASTER_PATHS:
  ...
  for prop in data.get('props',[]) or []:
   ...
   if adj:
    row['injury_status']=adj['severity'];row['injury_detail']=adj['detail'];row['injury_adjusted']=True;row['projected_minutes_pre_injury']=row.get('projected_minutes');row['projected_minutes']=adj['projected_minutes'];row['minutes_delta']=adj['minutes_delta'];row['usage_delta']=adj.get('usage_delta',0);row['injury_projection_factor']=adj['projection_factor']
    projection=row.get('projection',row.get('proj'))
    if projection not in (None,''):
     before=sf(projection);newp=round(before*adj['projection_factor'],2);row['projection_pre_injury']=before;row['projection']=newp;row['proj']=newp;row['injury_projection_delta']=round(newp-before,2)
    conf=sf(row.get('confidence',row.get('final_score')),0);row['confidence_pre_injury']=conf;row['confidence']=max(0,round(conf-adj['confidence_penalty'],1));row['injury_confidence_delta']=round(row['confidence']-conf,1)
    if adj['is_out']:
 ...
 for path in DECISION_PATHS:
  ...
  if isinstance(data,list):data=guard_rows(data,adjustments)
   ...
   if 'summary' in data and isinstance(data['summary'],dict):data['summary']['injury_blocked']=sum(1 for k in ('qualified_bets','recommended_card') for r in data.get(k,[]) if r.get('blocked_reason'))
  dump(path,data)
