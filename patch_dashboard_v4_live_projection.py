...
SCRIPT=r'''<script id="v4-live-projection-script">(function(){window.liveProjection=function(){const p=DATA.live_projection||{},games=Array.isArray(p.games)?p.games:[];const cards=games.map(g=>{const players=(g.players||[]).slice(0,10).map(r=>{const d=r.distributions||{};return `<div class="livePlayer"><b>${E(r.player)}</b><div class="small mono">MIN ${E(r.live_minutes)} → ${E(r.projected_final_minutes)}${r.foul_trouble?' · FOUL TROUBLE':''}</div><div class="small mono">PTS ${E(d.PTS?.mean??'—')} · REB ${E(d.REB?.mean??'—')} · AST ${E(d.AST?.mean??'—')} · PRA ${E(d.PRA?.mean??'—')}</div><div class="liveRange mono">PTS ${E(d.PTS?.p10??'—')}–${E(d.PTS?.p90??'—')} · Live weight ${(Number(r.live_weight||0)*100).toFixed(0)}%</div></div>`}).join('');return `<div class="liveCard"><div><span class="livePulse"></span><b>${E(g.game)}</b></div><div class="small mono">Q${E(g.period)} ${E(g.clock)} · ${E(g.away_score)}-${E(g.home_score)}</div><div class="row"><div><div class="small">Final Total</div><div class="liveScore mono">${E(g.projected_final_total)}</div></div><div><div class="small">Final Margin</div><div class="liveScore mono">${E(g.projected_final_margin)}</div></div></div>${players}</div>`}).join('');return `<div class="section"><h2 class="mono">Live In-Game Projections</h2><div class="small mono">Pregame priors updated with score, clock, pace, minutes, usage, and foul state.</div><div class="liveGrid">${cards||'<div class="empty mono">No WNBA games are currently in progress.</div>'}</div></div>`};})();</script>'''
...
def main():
 ...
 HTML.write_text(html,encoding='utf-8');print('Live In-Game Projections added')
