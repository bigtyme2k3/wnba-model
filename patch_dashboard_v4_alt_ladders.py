...
function sourceGames(){const a=Array.isArray(DATA.master?.games)?DATA.master.games:[];const b=Array.isArray(DATA.games)?DATA.games:[];return a.length?a:b}
function gameName(g){return String(g?.game||g?.matchup||`${g?.away_team||g?.away||''} @ ${g?.home_team||g?.home||''}`).trim()}
function activeGameSet(){return new Set(sourceGames().map(gameName).map(norm).filter(Boolean))}
...
window.altLadders=function(){
 ...
 const note=active.size
  ?`${games.length} active game${games.length===1?'':'s'} · ${excluded} off-slate player${excluded===1?'':'s'} excluded · generated for the current Games tab`
  :'No active-slate game identity was available, so historical ALT ladders are hidden instead of mixed into today’s card.';
...
def main():
    ...
    print('Sportsbook ALT ladders filtered to active daily slate')
