...
def empty_dashboard_data():
    ...
    return {
        ...
        "props_board": [],
        "data_health": {
            "odds": "missing", "props": "missing", "player_points": "missing",
            "spreads_found": 0, "totals_found": 0, "props_found": 0,
            "player_points_found": 0, "games": 0, "actionable_bets": 0,
            "high_bets": 0, "last_updated_utc": None,
        },
...
def load_player_points_for_date(target_date):
    candidates = [os.path.join(RAW_DIR, f"player_points_{target_date}.csv"), os.path.join(RAW_DIR, "player_points_today.csv")]
    for path in candidates:
        if os.path.exists(path):
            try:
                df = pd.read_csv(path)
...
def enrich_with_player_points(data):
    ...
    if not points:
        ...
        health["player_points"] = health.get("player_points", "missing")
        health["player_points_found"] = 0
        return data
    ...
    health["props_found"] = max(int(health.get("props_found", 0) or 0), len(points))
    return data
