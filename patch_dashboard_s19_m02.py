...
@media(max-width:900px){{.s19count{{width:100%;margin-left:0}}.s19scroll{{max-height:68vh}}}}
</style>
<script id="s19-m02-script">
(function(){{
 const D={raw}; window.WNBA_S19_M02=D; let propFilter='ALL';
 ...
 function propsHtml(){{const all=D.player_props||[];const counts={{OVER:0,UNDER:0,PASS:0,ACTIONABLE:0}};all.forEach(r=>{{const p=String(r.recommendation||'PASS').toUpperCase();counts[p]=(counts[p]||0)+1;if(r.eligible)counts.ACTIONABLE++}});const rows=all.filter(r=>propFilter==='ALL'||(propFilter==='ACTIONABLE'&&r.eligible)||String(r.recommendation||'PASS').toUpperCase()===propFilter);const btn=f=>`<button class="s19filter ${{propFilter===f?'a':''}}" onclick="window.s19SetPropFilter('${{f}}')">${{f==='ALL'?'All '+all.length:f+' '+(counts[f]||0)}}</button>`;return `<div class="section">${{gate()}}<h2 class="mono">Player Props · Model Predictions</h2><div class="s19muted mono">Exact current-slate sportsbook lines vs current V5 projections. Player and prop stay pinned while the rest of the columns scroll horizontally.</div><div class="s19toolbar">${{['ALL','OVER','UNDER','PASS','ACTIONABLE'].map(btn).join('')}}<span class="s19count">Showing ${{rows.length}} of ${{all.length}}</span></div><div class="s19scroll"><table class="s19table"><thead><tr><th>Player</th><th>Prop</th><th>Line</th><th>Model</th><th>Edge</th><th>Pick</th><th>Confidence</th><th>Minutes</th><th>Injury</th><th>Market O/U</th></tr></thead><tbody>${{rows.map(r=>{{const pick=String(r.recommendation||'PASS');const cls=pick==='OVER'?'s19good':pick==='UNDER'?'s19under':'s19pass';return `<tr><td><b>${{esc(r.player)}}</b><div class="s19muted">${{esc(r.team||'')}} · ${{esc(r.game)}}</div></td><td>${{esc(r.stat)}}</td><td>${{num(r.line)}}</td><td><b>${{num(r.model_projection)}}</b></td><td class="${{Number(r.edge)>=0?'s19good':'s19under'}}">${{Number(r.edge)>=0?'+':''}}${{num(r.edge)}}</td><td class="${{cls}}"><b>${{esc(pick)}}</b>${{r.eligible?'':'<div class="s19muted">not actionable</div>'}}</td><td>${{r.confidence==null?esc(r.confidence_label||'—'):num(r.confidence)}}</td><td>${{r.projected_minutes==null?'—':num(r.projected_minutes)}}${{r.minutes_delta==null?'':`<div class="s19muted">Δ ${{Number(r.minutes_delta)>=0?'+':''}}${{num(r.minutes_delta)}}</div>`}}</td><td>${{esc(r.injury_status||'CLEAR')}}${{r.injury_adjusted?'<div class="s19muted">adjusted</div>':''}}</td><td>O ${{esc(r.best_over_book||'Consensus')}} ${{esc(r.best_over_price)}}<br>U ${{esc(r.best_under_book||'Consensus')}} ${{esc(r.best_under_price)}}</td></tr>`}}).join('')}}</tbody></table></div></div>`}}
 window.s19SetPropFilter=function(f){{propFilter=f;const r=root();if(r)r.innerHTML=propsHtml()}};
