...
def ensure_predictions():
    ...
    if audit.get('injury_source_verified') is not True:
        if audit.get('recommendations_actionable') is not False:
            raise SystemExit('Unverified injury source did not disable recommendations')
        if int(audit.get('actionable_injury_unverified_props') or 0) != 0:
            raise SystemExit('Unverified injury source left actionable Player Props')
        if int(audit.get('best_bets_current_rows') or 0) != 0 or int(audit.get('portfolio_current_rows') or 0) != 0:
            raise SystemExit('Unverified injury source left Best Bets or Portfolio rows')
    return payload
...
(function(){{
 ...
 const injuryVerified=()=>D.injury_source_verified===true;
 function injuryBadges(ctx){{ctx=ctx||{{}};const parts=[];if(ctx.out)parts.push(`<span class="s19badge s19bad">${{ctx.out}} OUT</span>`);if(ctx.questionable)parts.push(`<span class="s19badge s19warn">${{ctx.questionable}} Q</span>`);if(ctx.probable)parts.push(`<span class="s19badge">${{ctx.probable}} PROB</span>`);return parts.join('')||(injuryVerified()?'<span class="s19badge">No listed injuries</span>':'<span class="s19badge s19warn">Official injury report unverified</span>')}}
 function gamesHtml(){{const games=D.games||[],intro=injuryVerified()?'Official injury report verified for this slate.':'Official injury report is not available yet. Projections and lines are visible for research; all recommendations remain withheld.';return `<div class="section">${{gate()}}<h2 class="mono">Slate Games · Model Projections</h2><div class="s19muted mono">${{intro}}</div><div class="s19grid" style="margin-top:12px">${{games.map(g=>{{const p=g.projection||{{}},m=g.market||{{}},e=g.edge||{{}},r=g.recommendation||{{}},u=g.model_recommendation_unverified||{{}},c=g.injury_context||{{}},withheld=g.recommendation_gate==='INJURY_SOURCE_UNVERIFIED';return `<div class="s19card"><div class="s19muted mono">${{esc(g.game)}}</div><h3 class="mono">${{esc(g.away_team)}} ${{num(p.away_score)}} · ${{esc(g.home_team)}} ${{num(p.home_score)}}</h3>${{injuryBadges(c)}}<div class="s19metrics"><div class="s19metric">Win Probability<b>${{esc(g.home_team)}} ${{p.home_win_probability==null?'—':(Number(p.home_win_probability)*100).toFixed(1)+'%'}}</b></div><div class="s19metric">Spread<b>Book ${{esc(m.home_spread)}} · Model ${{num(p.model_home_spread)}}</b><span class="s19good">Edge ${{num(e.spread)}}</span></div><div class="s19metric">Total<b>Book ${{esc(m.total)}} · Model ${{num(p.total)}}</b><span class="s19good">Edge ${{num(e.total)}}</span></div><div class="s19metric">Recommendation<b>${{esc(r.spread||'PASS')}} · ${{esc(r.total||'PASS')}}</b>Conf ${{esc(g.confidence)}} · Grade ${{esc(g.model_grade)}}${{withheld?`<div class="s19muted">Diagnostic lean ${{esc(u.spread||'PASS')}} · ${{esc(u.total||'PASS')}} · not actionable</div>`:''}}</div></div><div class="s19muted" style="margin-top:9px">Injury source ${{esc(c.generated_at_utc||D.injury_generated_at_utc)}} · ${{injuryVerified()?'verified':'unverified'}}</div></div>`}}).join('')}}</div></div>`}}
 ...
 function propsHtml(){{const all=D.player_props||[],isActionable=r=>r.eligible_for_bet===true&&String(r.final_action||'').toUpperCase()==='BET',counts={{OVER:0,UNDER:0,PASS:0,ACTIONABLE:0}};all.forEach(r=>{{const p=String(r.recommendation||'PASS').toUpperCase();counts[p]=(counts[p]||0)+1;if(isActionable(r))counts.ACTIONABLE++}});const filtered=all.filter(r=>propFilter==='ALL'||(propFilter==='ACTIONABLE'&&isActionable(r))||String(r.recommendation||'PASS').toUpperCase()===propFilter);const rows=sortedRows(filtered);const btn=f=>`<button class="s19filter ${{propFilter===f?'a':''}}" onclick="window.s19SetPropFilter('${{f}}')">${{f==='ALL'?'All '+all.length:f+' '+(counts[f]||0)}}</button>`;const sortStatus=propSort.key?` · Sorted ${{esc(propSort.key)}} ${{propSort.dir==='asc'?'↑':'↓'}}`:'',injuryNotice=injuryVerified()?'Official injury report verified.':'Official injury report unverified · all model leans are not actionable.';return `<div class="section">${{gate()}}<h2 class="mono">Player Props · Model Predictions</h2><div class="s19muted mono">Exact slate sportsbook lines vs current V5 projections. ${{injuryNotice}} Tap any column heading to sort ascending/descending.</div><div class="s19toolbar">${{['ALL','OVER','UNDER','PASS','ACTIONABLE'].map(btn).join('')}}<span class="s19count">Showing ${{rows.length}} of ${{all.length}}${{sortStatus}}</span></div><div class="s19scroll"><table class="s19table"><thead><tr><th>${{sortHead('Player','player')}}</th><th>${{sortHead('Prop','stat')}}</th><th>${{sortHead('Line','line')}}</th><th>${{sortHead('Model','model')}}</th><th>${{sortHead('Edge','edge')}}</th><th>${{sortHead('Pick','pick')}}</th><th>${{sortHead('Confidence','confidence')}}</th><th>${{sortHead('Minutes','minutes')}}</th><th>${{sortHead('Injury','injury')}}</th><th>${{sortHead('Market O/U','market')}}</th></tr></thead><tbody>${{rows.map(r=>{{const pick=String(r.recommendation||'PASS'),actionable=isActionable(r),state=actionable?'BET approved':(pick==='PASS'?'withheld · not actionable':'model lean · not actionable'),cls=actionable?(pick==='OVER'?'s19good':'s19under'):'s19pass';return `<tr><td><b>${{esc(r.player)}}</b><div class="s19muted">${{esc(r.team||'')}} · ${{esc(r.game)}}</div></td><td>${{esc(r.stat)}}</td><td>${{num(r.line)}}</td><td><b>${{num(r.model_projection)}}</b></td><td class="${{Number(r.edge)>=0?'s19good':'s19under'}}">${{Number(r.edge)>=0?'+':''}}${{num(r.edge)}}</td><td class="${{cls}}"><b>${{esc(pick)}}</b><div class="s19muted">${{state}}</div></td><td>${{r.confidence==null?esc(r.confidence_label||'—'):num(r.confidence)}}</td><td>${{r.projected_minutes==null?'—':num(r.projected_minutes)}}${{r.minutes_delta==null?'':`<div class="s19muted">Δ ${{Number(r.minutes_delta)>=0?'+':''}}${{num(r.minutes_delta)}}</div>`}}</td><td>${{esc(r.injury_status||'CLEAR')}}${{r.injury_adjusted?'<div class="s19muted">adjusted</div>':''}}</td><td>O ${{esc(r.best_over_book||'Consensus')}} ${{esc(r.best_over_price)}}<br>U ${{esc(r.best_under_book||'Consensus')}} ${{esc(r.best_under_price)}}</td></tr>`}}).join('')}}</tbody></table></div></div>`}}
