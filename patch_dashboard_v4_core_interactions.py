...
(function(){
  ...
  function rowDate(r){return String(r?.game_date||r?.target_date||r?.commence_time||r?.start_time||'').slice(0,10)}
  function isCurrentGameRow(r,g){
    if(!sameGame(r?.game||r?.matchup||r?.event,gameName(g)))return false;
    const d=rowDate(r), gd=String(g?.game_date||g?.start_time||'').slice(0,10);
    return !d||!gd||d===gd;
  }
  function matchingRows(g){
    const pools=[DATA.market,DATA.projection,DATA.monte];
    return pools.flatMap(rowsFrom).filter(r=>isCurrentGameRow(r,g));
  }
  ...
  function hasModelProjection(rows){return rows.some(r=>['projected_margin','predicted_margin','margin_prediction','model_spread','spread_projection','projected_total','predicted_total','total_projection','model_total','pred'].some(k=>r?.[k]!==undefined&&r?.[k]!==null&&r?.[k]!==''))}
  function topPicks(g,modelReady){
    if(!modelReady)return[];
    const candidates=arr(DATA.master?.best_bets).concat(arr(DATA.master?.props)).filter(r=>isCurrentGameRow(r,g));
  ...
  function spreadLabel(g,spread){
    if(spread==='-'||spread===null||spread===undefined||spread==='')return'-';
    const n=Number(spread); if(Number.isNaN(n))return String(spread);
    const team=n<0?g.home_team:g.away_team;
    const signed=n>0?`+${n}`:String(n);
    return `${team||''} ${signed}`.trim();
  }
  function totalLabel(total){const n=Number(total);return Number.isFinite(n)?n.toFixed(1):val(total)}
  function predictionCard(g){
    const name=gameName(g), rows=matchingRows(g), modelReady=hasModelProjection(rows), picks=topPicks(g,modelReady);
    const rawSpread=val(g.spread??g.spread_home,firstValue(rows,['spread','market_spread','consensus_spread']));
    const rawTotal=val(g.total,firstValue(rows,['total','market_total','consensus_total']));
    const marketSpread=spreadLabel(g,rawSpread), marketTotal=totalLabel(rawTotal);
    ...
    const spreadPick=firstValue(rows,['spread_pick','ats_pick','spread_lean','predicted_winner'],projectedMargin!=='-'?`Model margin ${projectedMargin}`:'Awaiting current model projection');
    const totalPick=firstValue(rows,['total_pick','ou_pick','total_lean'],projectedTotal!=='-'&&rawTotal!=='-'?(Number(projectedTotal)>Number(rawTotal)?'OVER':'UNDER'):'Awaiting current model projection');
    ...
    const pickHtml=!modelReady?'<div class="previewNotice mono">Current market data is loaded, but today\'s model projections are not available yet. Player picks are withheld so stale recommendations are not shown.</div>':(picks.length?picks.map((p,i)=>`<div class="previewPick"><div class="previewPickRank mono">Top Pick ${i+1}</div><div class="previewPickTitle">${esc(val(p.player,'Game'))} ${esc(val(p.stat,p.market||''))} ${esc(val(p.signal||p.side,p.pick||''))}</div><div class="small mono">Line ${esc(val(p.line||p.consensus_line))} · ${esc(val(p.book||p.best_book||p.sportsbook))}</div><div class="chip mono">Score ${esc(val(p.final_score||p.confidence||p.edge_pct))}</div></div>`).join(''):'<div class="empty mono">No qualified game picks for this matchup.</div>');
    const source=val(g.source,'market feed');
    return `<div class="previewPanel"><div class="previewHead"><div><div class="label mono">Quick Game Preview</div><div class="previewTitle mono">${esc(name)}</div><div class="small mono">${esc(val(g.start_time,'Time TBD'))} · ${esc(val(g.status,'Pregame'))}</div></div><button class="previewBtn" onclick="closeGamePreview()">Close</button></div><div class="previewGrid"><div class="previewStat"><div class="label mono">Market Spread</div><div class="previewValue mono">${esc(marketSpread)}</div><div class="small mono">Prediction: ${esc(spreadPick)} ${spreadProb!=='-'?'· '+esc(pct(spreadProb)):''}</div><div class="marketMeta mono">Source: ${esc(source)}</div></div><div class="previewStat"><div class="label mono">Projected Margin</div><div class="previewValue mono">${esc(projectedMargin)}</div><div class="small mono">Model spread estimate</div></div><div class="previewStat"><div class="label mono">Market Total</div><div class="previewValue mono">${esc(marketTotal)}</div><div class="small mono">Consensus market total · Prediction: ${esc(totalPick)} ${totalProb!=='-'?'· '+esc(pct(totalProb)):''}</div></div><div class="previewStat"><div class="label mono">Projected Total</div><div class="previewValue mono">${esc(projectedTotal)}</div><div class="small mono">Model scoring estimate</div></div></div><h3 class="mono">Top 3 Game Picks</h3><div class="previewPickGrid">${pickHtml}</div><div class="previewActions"><button class="previewBtn" onclick="openGameProps('${js(name)}')">View Player Props</button><button class="previewBtn" onclick="render('best')">Open Best Bets</button></div></div>`;
  }
