...
(function(){
  ...
  function draw(){
    ...
    let table=rows.length?`<div class="props-scroll"><div class="props-table"><div class="props-head"><div>Player</div><div>Stat</div><div>Best Line</div><div>Over/Yes</div><div>Under/No</div><div>Projected</div><div>Last 5</div><div>L5 Hit</div><div>L10 Hit</div><div>H2H L5</div><div>Opp Rank</div></div>${rows.map(p=>`<article class="prop-row ${p.conf||'LOW'}"><div><div class="player-name">${safe(p.player)}</div><div class="player-meta">${safe(p.best_book_title||p.best_book,'Best book —')} · ${safe(p.injury_status,'ACTIVE')} · ${safe(p._game)}</div></div><div><span class="stat-pill ${p.conf==='HIGH'?'high':''}">${safe(p.stat)}</span></div><div class="board-value">${showLine(p)}</div><div class="signal-over">${p.signal==='OVER'||p.signal==='YES'?p.signal:'—'}</div><div class="signal-under">${p.signal==='UNDER'||p.signal==='NO'?p.signal:'—'}</div><div class="${p.line?'proj-bright':'proj-dim'}">${p.stat==='DD'||p.stat==='TD'?Math.round((p.pred||0)*100)+'%':safe(p.pred)}</div><div>${boxes(p)}</div><div>${p.signal?pct(p.last5_hit)+' '+p.signal:'—'}</div><div>${p.signal?pct(p.last10_hit)+' '+p.signal:'—'}</div><div>${arr(p.h2h_last5).length?arr(p.h2h_last5).join(', '):'—'}</div><div class="${rank(p.opp_rank)}">${safe(p.opp_rank)}</div></article>`).join('')}</div></div>`:'<div class="empty">No sportsbook props for this selected game/stat filter. Try ALL or another stat.</div>';
