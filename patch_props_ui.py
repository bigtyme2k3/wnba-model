...
(function(){
  ...
  function draw(){
    ...
    let sortbar=sorts.map(([k,l])=>`<button class="filter-btn ${curSort===k?'active':''}" onclick="setPropsSort('${k}')">${l}</button>`).join('');
...
<style id="props-ui-fix-css">
  ...
  .l5-wrap.betsy{display:flex;gap:7px;align-items:center}
  .l5-box{text-align:center}.l5-num{min-width:34px;height:34px;border-radius:7px;display:flex;align-items:center;justify-content:center;font-weight:800}.l5-opp{font-size:10px;opacity:.65;margin-top:3px}
  .l5-hit{background:rgba(0,229,160,.22);color:#00e5a0;border:1px solid rgba(0,229,160,.35)}
  .l5-miss{background:rgba(248,113,113,.20);color:#f87171;border:1px solid rgba(248,113,113,.28)}
  .l5-neutral{background:rgba(148,163,184,.12);color:#94a3b8;border:1px solid rgba(148,163,184,.18)}
  .hit-cell{font-weight:800;line-height:1.25;text-align:center}
