...
function rows(){const p=(window.DATA&&DATA.alt_streaks)||{},gs=games(),allowed=new Set(['fanduel','draftkings','fanatics']);return (Array.isArray(p.rows)?p.rows:[]).filter(r=>r.line_type==='alternate'&&allowed.has(String(r.best_book||'').toLowerCase())&&(!r.game||gs.some(g=>g.game===r.game)))}
...
window.WNBA_ALT_PROPS_TABLE={version:'1.2',style:'sportsbook-line-streak-table',exact_market_rows:true,allowed_sportsbooks:['FanDuel','DraftKings','Fanatics'],sortable_columns:columns.map(c=>c[0])};
})();</script>'''
