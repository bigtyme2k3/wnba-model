...
function panel(){const raw=D(),p=raw.alt_performance||raw||{},s=p.summary||{};return `<section id="alt-props-performance-panel" class="altPerfPanel"><div class="altPerfPanelHead"><div><div class="altPerfPanelTitle mono">ALT Props Performance</div><div class="altPerfPanelNote mono">Frozen pregame ALT selections graded against verified results.</div></div></div><div class="altPerfGrid2"><div class="altPerfMetric"><div class="altPerfMetricLabel">Archived</div><div class="altPerfMetricValue">${esc(s.archived_candidates??0)}</div></div><div class="altPerfMetric"><div class="altPerfMetricLabel">Graded</div><div class="altPerfMetricValue">${esc(s.graded??0)}</div></div><div class="altPerfMetric"><div class="altPerfMetricLabel">Record</div><div class="altPerfMetricValue">${esc(s.wins??0)}-${esc(s.losses??0)}-${esc(s.pushes??0)}</div></div><div class="altPerfMetric"><div class="altPerfMetricLabel">Hit Rate</div><div class="altPerfMetricValue">${pct(s.hit_rate)}</div></div><div class="altPerfMetric"><div class="altPerfMetricLabel">P/L</div><div class="altPerfMetricValue ${cls(s.profit_loss_units)}">${Number(s.profit_loss_units||0).toFixed(2)}u</div></div><div class="altPerfMetric"><div class="altPerfMetricLabel">ROI</div><div class="altPerfMetricValue ${cls(s.roi)}">${pct(s.roi)}</div></div><div class="altPerfMetric"><div class="altPerfMetricLabel">Threshold</div><div class="altPerfMetricValue">${esc(s.recommended_minimum_score_band??'—')}</div></div><div class="altPerfMetric"><div class="altPerfMetricLabel">Calibration</div><div class="altPerfMetricValue">${s.calibration_ready?'Ready':'Collecting'}</div></div></div><div class="altPerfBreakouts">${mini('By Score Band',p.by_score_band)}${mini('By Stat',p.by_stat)}${mini('By Side',p.by_side)}${mini('By Sportsbook',p.by_sportsbook)}</div></section>`}
...
window.WNBA_ALT_PROPS_PERFORMANCE_PANEL={version:'1.0',location:'below-alt-props-table'};
})();</script>'''
    ...
    print({'status':'PASS','alt_props_performance':'below-table','data':DATA.exists()})
