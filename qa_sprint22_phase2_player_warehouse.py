...
def run():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        root=Path(tmp); raw=root/'raw'; out=root/'out'; raw.mkdir()
        pd.DataFrame([
        ...
        logs=[]
        for i in range(12):
            logs.append({'Player':'A Player','Team':'Aces','Opponent':'Liberty','Date':f'2026-07-{i+1:02d}','MIN':30+i%3,'PTS':10+i,'REB':4+i%4,'AST':3+i%2})
        pd.DataFrame(logs).to_csv(raw/'player_game_logs_2026.csv',index=False)
        pd.DataFrame([{'Player':'A Player','Team':'Aces','Date':'2026-07-12','Status':'probable'}]).to_csv(raw/'injuries_2026.csv',index=False)
