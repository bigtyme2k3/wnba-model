...
(function(){
 ...
 window.remainingSeasonView=function(){
  ...
  const state=String(d.status||'unknown').toUpperCase(),coverage=String(d.coverage||'unknown');
  const sourceNote=`<div class="small mono">Status ${e(state)} · ${e(coverage)} · source ${e(d.source||'—')}</div>`;
  ...
  return `<div class="section"><h2 class="mono">Remaining Season Intelligence</h2>${sourceNote}<div class="rsGrid"><div class="rsCard"><div class="small">Remaining games</div><b>${e(s.remaining_games||0)}</b></div><div class="rsCard"><div class="small">Teams tracked</div><b>${e(s.teams||0)}</b></div><div class="rsCard"><div class="small">First game</div><b>${e((s.first_game_utc||'').slice(0,10)||'—')}</b></div><div class="rsCard"><div class="small">Last game</div><b>${e((s.last_game_utc||'').slice(0,10)||'—')}</b></div></div></div><div class="section"><h3 class="mono">Schedule Difficulty</h3>${rows||'<div class="empty mono">Schedule data not loaded.</div>'}</div><div class="section"><h3 class="mono">Next Games</h3>${g.slice(0,20).map(x=>`<div class="prodGate"><span>${e(x.away)} @ ${e(x.home)}</span><b>${e((x.date_utc||'').replace('T',' ').slice(0,16))} UTC</b></div>`).join('')}</div></div>`;
 };
