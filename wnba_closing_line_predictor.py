...
def run():
    ...
    for t in timelines:
        obs=t.get('observations') or []
        if not obs:continue
        row=obs[-1]; m=moves.get(t['market_id'],{}); s=sigs.get(t['market_id'],{}); mins=(parse(t['commence_time_utc'])-parse(row['snapshot_time_utc'])).total_seconds()/60; k=key(t,row,mins,m,s); model=models.get(k); cp=num(row.get('point')); cprob=implied(row.get('american_price'))
        rec={'market_id':t['market_id'],'event_id':t['event_id'],'commence_time_utc':t['commence_time_utc'],'home_team':t['home_team'],'away_team':t['away_team'],'bookmaker':t['bookmaker'],'market':t['market'],'participant':t.get('participant'),'selection':t.get('selection'),'minutes_to_tip':round(mins,2),'current_point':cp,'current_price':row.get('american_price'),'forecast_status':'MODEL_READY' if model else 'INSUFFICIENT_HISTORY','sample_size':0 if not model else model['sample_size']}
        if model:
