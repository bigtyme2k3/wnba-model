...
def engineer(rows:List[Dict[str,Any]])->tuple[List[Dict[str,Any]],Dict[str,Any]]:
    ...
    # Process each date as a batch. Outcomes from one record can never become
    # features for another record on the same date.
    for _, date_rows_iter in groupby(ordered,key=lambda r:str(r.get('date') or '')):
        ...
        for row in date_rows:
            ...
            if result in {'WIN','LOSS'}:
                pending.append((player,mkt,1 if result=='WIN' else 0))

        # Only after all rows for the date are engineered do their outcomes
        # become eligible for subsequent dates.
        for player,mkt,y in pending:
            if player: player_hist[player].append(y)
            market_hist[mkt].append(y)

    manifest={'generated_at_utc':datetime.now(timezone.utc).isoformat(),'history_records':len(rows),'engineered_records':len(output),'engineered_features':sorted(created),'engineered_feature_count':len(created),'leakage_policy':'Only current-row pregame fields and outcomes from strictly earlier dates are used.'}
