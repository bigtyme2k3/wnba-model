...
- every legacy/canonical discrepancy is checked against repository-local raw boxscores.
"""
...
def load_all_raw():
    rows=[]
    for p in sorted(RAW_DIR.glob('boxscores_*.csv')):
        try:
...
def main():
    ...
    for d in cert.get('discrepancy_samples') or []:
        ...
        confirmations.append({'archive_index':idx,'player':d.get('player'),'game_id':gid,'stat':c.get('stat'),'raw_actual':rv,'canonical_actual':ca,'raw_confirms_canonical':ok,'raw_source':None if rr is None else rr.get('_source_file')})
    ...
    cert['phase11_raw_boxscore_files_scanned']=len(list(RAW_DIR.glob('boxscores_*.csv')))
