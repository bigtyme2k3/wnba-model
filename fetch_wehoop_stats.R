...
safe_scoreboard <- function() {
  ...
  bind_rows(Filter(function(x) !is.null(x) && nrow(x) > 0, parts))
}
...
for (gid in missing_ids) {
  ...
  box <- tryCatch(espn_wnba_player_box(gid), error = function(e) {
    warning("wehoop player box failed for ", gid, ": ", conditionMessage(e))
    NULL
  })
  if (!is.null(box) && nrow(box) > 0) new_boxes[[length(new_boxes) + 1]] <- box
  Sys.sleep(0.15)
}

all_boxes <- bind_rows(existing, bind_rows(new_boxes)) %>%
  mutate(game_id = as.character(game_id), game_date = as.Date(game_date)) %>%
...
status <- list(
  ...
  new_games_fetched = length(missing_ids),
  generated_at_utc = format(Sys.time(), tz = "UTC", usetz = TRUE)
)
write_json(status, file.path(out_dir, "wehoop_stats_status.json"), pretty = TRUE, auto_unbox = TRUE)
message("✓ wehoop stats complete: ", nrow(normalized), " player-game rows, ", nrow(players), " players")
