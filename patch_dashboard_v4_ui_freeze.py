...
function decorateGames(){const root=document.getElementById('root');if(!root||document.getElementById('games-injury-intelligence'))return;const html=injurySummary();if(!html)return;const section=root.querySelector('.section')||root.firstElementChild;if(section)section.insertAdjacentHTML('afterbegin',html);else root.insertAdjacentHTML('afterbegin',html)}
window.render=function(view='games'){
 const aliases={today:'games',gameperf:'game-performance',alt:'alt-props',altstreaks:'alt-props',altperf:'alt-props',books:'sportsbooks',injuries:'games',performance:'game-performance'};view=aliases[view]||view;if(!NAV.some(x=>x[0]===view))view='games';chrome(view);document.body.classList.toggle('show-system-health',view==='health');const root=document.getElementById('root');if(!root)return;let html='';
...
window.WNBA_V4_UI_FREEZE={version:'1.0',tabs:NAV.map(x=>x[0]),injuries_location:'games',alt_props_tab:true};
window.render('games');
})();</script>'''
...
def main() -> None:
    ...
    print({'status':'PASS','tabs':13,'alt_props':True,'injuries_in_games':True,'standalone_injuries':False})
