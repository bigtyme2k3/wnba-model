...
MASTER_DIR="data/master"; DASH_DIR="data/dashboard"; RAW_DIR="data/raw"; CONFIG="config/source_registry.json"
...
def team_name(v:Any)->str:
    if isinstance(v,dict): return str(v.get('name') or v.get('abbreviation') or v.get('abbr') or '')
    return str(v or '')
...
def parse_game_text(g:str):
    if ' @ ' in g: return g.split(' @ ',1)
    if ' at ' in g: return g.split(' at ',1)
    if ' vs ' in g:
        home,away=g.split(' vs ',1); return away,home
    return '',''
...
def prediction_games(target:str):
    ...
    for g in pred.get('games',[]):
        away=team_name(g.get('away')) or g.get('away_team',''); home=team_name(g.get('home')) or g.get('home_team','')
...
def odds_games(target:str):
    ...
    for r in rows:
        ...
        latest[key]={'game_id':r.get('game_id') or key,'game_date':target,'bucket':'today','game':display_game(away,home),'away_team':away,'home_team':home,'away_score':'','home_score':'','status':'Pregame','start_time':r.get('commence_time') or '', 'spread':clean_num(r.get('spread_home')),'total':clean_num(r.get('total')),'moneyline_home':clean_num(r.get('ml_home')),'moneyline_away':clean_num(r.get('ml_away')),'source':src or 'odds_csv'}
...
def projection_for_stat(p:dict, stat:str):
    stat=(stat or '').upper()
    pts=as_float(p.get('roll5_pts'),as_float(p.get('ppg'),0)) or 0
    reb=as_float(p.get('roll5_reb'),as_float(p.get('rpg'),0)) or 0
    ast=as_float(p.get('roll5_ast'),as_float(p.get('apg'),0)) or 0
    threes=as_float(p.get('roll5_threes'),0) or 0
...
def enrich_prop(m:dict, players_by_name:dict):
    p=players_by_name.get(str(m.get('player','')).lower(),{})
    line=as_float(m.get('consensus_line'))
    proj=projection_for_stat(p,m.get('stat')) if p else ''
    edge=round(as_float(proj,0)-line,2) if proj!='' and line is not None else ''
    side='OVER' if edge!='' and edge>0.25 else 'UNDER' if edge!='' and edge<-0.25 else 'PASS'
    confidence=0
    if edge!='' and line:
        confidence=min(99, round(50+abs(edge)*8 + min(int(m.get('book_count',0))*3,12),1))
...
def props_from_consensus(players:list[dict]):
    sb=load_json(f'{DASH_DIR}/wnba_sportsbook_consensus.json',{})
    by_name={str(p.get('player','')).lower():p for p in players}
    return [enrich_prop(m,by_name) for m in sb.get('markets',[])]
...
def build(target:str):
    ...
    players=player_stats_from_live(); props=props_from_consensus(players); best_bets=build_best_bets(props)
    ...
    health=[source_status('sportsdataverse','optional_package_'+optional_package_status('sportsdataverse'),len(today_games),'Schedule source planned; predictions schedule is current source of truth.'),source_status('nba_api','optional_package_'+optional_package_status('nba_api'),len(players),'Advanced stats falls back to owned boxscore warehouse.'),source_status('boxscore_warehouse','ok' if players else 'missing',len(players),'wnba_players_live.json generated from official stats or boxscore fallback.'),source_status('odds_pipeline','ok' if props else 'missing',len(props),'sportsbook consensus markets normalized into master props.'),source_status('the_odds_api','backup_only',0,'Do not use as primary because credits burn quickly.'),source_status('litellm','optional_package_'+optional_package_status('litellm'),0,'AI gateway planned; not required for current dashboard.'),source_status('optuna','optional_package_'+optional_package_status('optuna'),0,'Model tuning planned; not required for current dashboard.')]
    master={'generated_at_utc':datetime.now(timezone.utc).isoformat(),'target_date':target,'schema_version':'master-v4','registry':registry,'summary':{'games':len(games),'today_games':len(today_games),'yesterday_games':len(yesterday_games),'players':len(players),'props':len(props),'best_bets':len(best_bets),'sportsbook_markets':odds.get('summary',{}).get('markets',0),'books':odds.get('summary',{}).get('books_detected',[])},'games':games,'players':players,'props':props,'best_bets':best_bets,'odds_summary':odds.get('summary',{}),'stats_quality':stats_quality,'source_health':source_health,'source_matrix':health}
