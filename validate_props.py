...
def validate(target_date: str, raw_dir: str, strict: bool = False) -> tuple[dict, int]:
    raw_paths = [os.path.join(raw_dir, f"props_raw_{target_date}.csv"), os.path.join(raw_dir, "props_today.csv")]
    points_paths = [os.path.join(raw_dir, f"player_points_{target_date}.csv"), os.path.join(raw_dir, "player_points_today.csv")]
    ...
    if props_df.empty:
        health["warnings"].append("Raw props file is missing or empty.")
    else:
        if "source" in props_df.columns and props_df["source"].notna().any():
            health["source"] = str(props_df["source"].dropna().iloc[0])
        ...
        if "stat" in props_df.columns:
            ...
            points = props_df[props_df["stat_raw"].astype(str).str.lower().isin({"points", "player_points"})]
            health["points_props_rows"] = int(len(points))
        else:
    ...
    if props_df.empty:
        ...
        health["next_action"] = "Open the Scrape props step logs. The Odds API may not have player props posted yet for today's WNBA events."
    elif health["points_props_rows"] == 0:
        health["status"] = "no_points_props"
        health["next_action"] = "The props feed returned rows, but no player_points / pts market. Check stat types in props_today.csv."
        ...
        health["status"] = "ok"
        health["next_action"] = "Props pipeline is working. Dashboard should display player props."
...
def main():
    ...
    print(f"\nSaved → {out}")
    print("✅ Props validation passed." if health["status"] == "ok" else f"⚠️ Props validation status: {health['status']}")
