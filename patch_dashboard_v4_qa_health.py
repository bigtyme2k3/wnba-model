...
(function(){
  ...
  const issues=(items,kind)=>arr(items).length?`<ul class="${kind==='block'?'issueList':'warnList'}">${arr(items).map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:'<span class="small mono">None</span>';
  window.health=function(){
    ...
    const rows=mods.map(m=>`<tr>
      ...
      <td>${issues(m.blockers,'block')}${issues(m.warnings,'warn')}</td>
    </tr>`).join('');
    ...
    const blockerHtml=blockers.length?blockers.map(b=>`<div class="gameCard"><b class="mono qa-red">${esc(b.id)} · ${esc(b.module)}</b>${issues(b.items,'block')}</div>`).join(''):'<div class="healthEmpty mono">No release blockers detected.</div>';
...
def main() -> None:
    ...
    if 'id="v4-qa-health-style"' not in html:
        html = html.replace("</head>", CSS + "</head>")
    if 'id="v4-qa-health-script"' not in html:
