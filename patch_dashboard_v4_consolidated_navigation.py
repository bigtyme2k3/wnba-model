...
if(typeof window.A!=='function')window.A=function(v){return Array.isArray(v)?v:[]};
...
if(typeof window.score!=='function')window.score=function(row){if(!row||typeof row!=='object')return 0;const value=row.projected_score??row.score??row.confidence??row.final_score??row.model_score??0;const n=Number(value);return Number.isFinite(n)?n:0};
const NAV=[['games','Games'],['gameprops','Game Props'],['props','Player Props'],['altstreaks','ALT Streaks'],['altperf','ALT Performance'],['dailyedges','Daily Edges'],['ensemble','Ensemble'],['simulation','Simulation'],['best','Best Bets'],['portfolio','Portfolio'],['ai','AI Center'],['results','Results'],['performance','Performance'],['explainability','Explainability'],['market','Market Intelligence'],['injuries','Injuries'],['mission','Mission Control'],['health','V4 Health']];
...
window.render=function(view='games'){const aliases={today:'games',markets:'gameprops',books:'props',alt:'altstreaks',edges:'dailyedges',ensembleedges:'ensemble',model:'ai'};view=aliases[view]||view;if(!NAV.some(([id])=>id===view))view='games';document.body.classList.toggle('show-system-health',view==='health');chrome(view);const root=document.getElementById('root');if(!root)return;let html='';if(view==='games')html=invoke('Games',[window.gamesV25,window.games]);else if(view==='gameprops')html=invoke('Game Props',[window.marketsV25,window.gameProps]);else if(view==='props')html=invoke('Player Props',[canonicalProps]);else if(view==='altstreaks')html=invoke('ALT Streaks',[window.altStreaks]);else if(view==='altperf')html=invoke('ALT Performance',[window.altPerformance]);else if(view==='dailyedges')html=invoke('Daily Edges',[window.dailyEdges]);else if(view==='ensemble')html=invoke('Ensemble',[window.ensembleIntelligence]);else if(view==='simulation')html=invoke('Simulation',[window.unifiedSimulation]);else if(view==='best')html=invoke('Best Bets',[window.best,window.bestBets,window.topPlays]);else if(view==='portfolio')html=invoke('Portfolio',[window.portfolio]);else if(view==='ai')html=invoke('AI Center',[window.ai]);else if(view==='results')html=invoke('Results',[window.results]);else if(view==='performance')html=invoke('Performance',[window.projectionPerformance,window.fullGamePerformance]);else if(view==='explainability')html=invoke('Explainability',[window.explainability,window.topPlays,window.best]);else if(view==='market')html=invoke('Market Intelligence',[window.marketIntelligence]);else if(view==='injuries')html=invoke('Injury Intelligence',[window.injuryIntelligence]);else if(view==='mission')html=invoke('Mission Control',[window.missionControl]);else if(view==='health')html=invoke('V4 Health',[window.health]);root.innerHTML=html;if(view==='props'&&typeof window.drawProps==='function'){try{window.drawProps()}catch(error){root.innerHTML=fail('Player Props',error)}}window.scrollTo(0,0)};
window.WNBA_TAB_SMOKE_TEST=function(){const root=document.getElementById('root');const original=root?root.innerHTML:'';const failures=[];for(const [id,label] of NAV){try{window.render(id);const text=root?.textContent||'';if(!root||!root.innerHTML.trim()||text.includes('failed to render')||text.includes('is unavailable'))failures.push(label)}catch(error){failures.push(`${label}: ${String(error)}`)}}if(root)root.innerHTML=original;return {tabs:NAV.length,failures,passing:NAV.length-failures.length}};
window.WNBA_FINAL_ROUTER={version:'2.5',tabs:NAV.map(x=>x[0]),props_api:true,compatibility_helpers:['E','S','q','A','game','score','sortableLabel','sortLabel']};window.render('games')})();</script>'''
...
def main():
 ...
 try:
  from patch_dashboard_v4_unified_simulation import main as simulation_patch
  simulation_patch()
 except Exception as exc:print('Unified simulation dashboard warning:',exc)
 ...
 print('Canonical router repair batch installed with simulation and runtime smoke test')
