...
SCRIPT=r'''<script id="v4-consolidated-navigation-script">(function(){const NAV=[['today','Games'],['props','Player Props & Best Bets'],['portfolio','Portfolio'],['ai','AI Center'],['results','Results'],['health','System Health']];function safe(fn,fallback=''){try{return typeof fn==='function'?fn():fallback}catch(err){return `<div class="section"><div class="empty mono">Section unavailable: ${String(err)}</div></div>`}}function chrome(view){const tabs=document.getElementById('tabs');if(tabs)tabs.innerHTML=NAV.map(([id,label])=>`<button class="tab ${id===view?'a':''}" data-view="${id}" onclick="render('${id}')">${label}</button>`).join('')}function propsView(){const ranked=safe(window.best||best);const standard=safe(window.props||props);const streaks=safe(window.altStreaks);return `<div class="propsStack"><div class="section"><h2 class="mono">Best Player Prop Opportunities</h2><div class="navSectionNote mono">Ranked selections are a filtered view of the player-prop board, not a separate model. Scores are research grades, not win probabilities.</div>${ranked}</div>${standard}<div class="section"><div class="streakNote"><b>Streak Board</b><div class="small">Verified standard-line streak context is kept with Player Props. True alternate lines appear only when supplied by the odds source.</div></div></div>${streaks}</div>`}function aiView(){const ai=safe(window.ai||ai);return `<div class="section"><h2 class="mono">AI Center</h2><div class="navSectionNote mono">Research, explanations, trend discovery, and warehouse questions. Internal engines remain in the background.</div></div><div class="aiCenterGrid"><div>${ai}</div></div>`}function healthView(){const health=safe(window.health||health);return `<div class="section"><h2 class="mono">System Health</h2><div class="navSectionNote mono">Data freshness, workflow status, source health, API usage, and technical diagnostics.</div></div>${health}`}window.render=function(view='today'){const aliases={games:'today',performance:'results',model:'ai',alt:'props',altperf:'results',books:'props',best:'props'};view=aliases[view]||view;if(!NAV.some(([id])=>id===view))view='today';document.body.classList.toggle('show-system-health',view==='health');chrome(view);const root=document.getElementById('root');if(!root)return;if(view==='today')root.innerHTML=safe(window.games||games);else if(view==='props'){root.innerHTML=propsView();if(typeof drawProps==='function')drawProps()}else if(view==='portfolio')root.innerHTML=safe(window.portfolio||portfolio);else if(view==='ai')root.innerHTML=aiView();else if(view==='results')root.innerHTML=safe(window.results||results);else if(view==='health')root.innerHTML=healthView();window.scrollTo(0,0)};window.render('today')})();</script>'''
...
def main():
 ...
 print('Dashboard simplified to six user-facing tabs; player props and best bets combined')
