...
if(typeof window.sortableLabel!=='function')window.sortableLabel=function(key,label){if(typeof window.sortLabel==='function')return window.sortLabel(key,label);return label};
if(typeof window.sortLabel!=='function')window.sortLabel=function(key,label){return window.sortableLabel(key,label)};
...
const arr=v=>Array.isArray(v)?v:[];const esc=v=>window.E(v);const val=(v,d='—')=>v===undefined||v===null||v===''?d:v;window.activeGame=window.activeGame||'';
...
function invoke(label,candidates){for(const fn of candidates){try{if(typeof fn==='function'){const html=fn();if(typeof html==='string'&&html.trim())return html}}catch(error){return fail(label,error)}}return `<div class="section"><div class="routerError"><b>${esc(label)} is unavailable.</b><div class="small mono">Expected one of: ${esc(candidates.map(x=>x?.name||'renderer').join(', '))}</div></div></div>`}
...
def main():
 ...
 print('Canonical router, shared Player Props API, and runtime compatibility helpers installed')
