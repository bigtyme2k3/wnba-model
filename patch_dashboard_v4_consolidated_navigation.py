...
(function(){
  ...
  function safe(fn,fallback=''){
    try{return typeof fn==='function'?fn():fallback}catch(err){return `<div class="section"><div class="empty mono">Section unavailable: ${String(err)}</div></div>`}
  }
  function setChrome(view){
    const tabs=document.getElementById('tabs');
    if(tabs)tabs.innerHTML=NAV.map(([id,label])=>`<button class="tab ${id===view?'a':''}" data-view="${id}" onclick="render('${id}')">${label}</button>`).join('');
    const sub=document.getElementById('sub');
    if(sub&&typeof S==='function'&&typeof fmt==='function')sub.textContent=`Slate ${S(DATA.master?.target_date,'-')} · Updated ${fmt(DATA.generated_at_utc)}`;
    const badge=document.getElementById('badge');
    if(badge&&typeof S==='function'&&typeof summary==='function')badge.textContent=`V4 · ${S(summary().sportsbook_markets,0)} odds markets`;
  }
  ...
  function modelView(){
    ...
    const healthView=safe(window.health||health);
    return `<div class="section"><h2 class="mono">Model Center</h2><div class="navSectionNote mono">AI diagnostics, production health, and pipeline status.</div></div><div class="modelCenterGrid"><div>${aiView}</div><div>${healthView}</div></div>`;
  }
  window.render=function(view='today'){
    const aliases={games:'today',results:'performance',portfolio:'performance',ai:'model',health:'model',altperf:'performance',books:'props'};
    view=aliases[view]||view;
    if(!NAV.some(([id])=>id===view))view='today';
    setChrome(view);
    const root=document.getElementById('root');
    if(!root)return;
    if(view==='today')root.innerHTML=safe(window.games||games);
    else if(view==='props'){
      root.innerHTML=safe(window.props||props);
      if(typeof drawProps==='function')drawProps();
    }
    else if(view==='alt')root.innerHTML=safe(window.altStreaks,'<div class="section"><div class="empty mono">ALT Streaks unavailable.</div></div>');
    else if(view==='best')root.innerHTML=safe(window.best||best);
    else if(view==='performance')root.innerHTML=performanceView();
    else if(view==='model')root.innerHTML=modelView();
    window.scrollTo(0,0);
  };
  window.render('today');
})();
...
def main() -> None:
    ...
    if 'id="v4-consolidated-navigation-script"' in html:
        html = replace_block(html, '<script id="v4-consolidated-navigation-script">', '</script>', SCRIPT)
    else:
