...
def run():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp); raw = root / "raw"; out = root / "warehouse"
        raw.mkdir()
        pd.DataFrame([
        ...
        pd.DataFrame([
            {"start_time":"2026-07-01T23:00:00Z","home_team":"Aces","away_team":"Liberty","sportsbook":"fanduel","market":"spread"}
        ]).to_csv(raw / "odds_2026.csv", index=False)
        ...
        assert stored["total_rows"] == 6; tests += 1
