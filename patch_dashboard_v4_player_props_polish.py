...
.propMeta{display:flex;justify-content:space-between;gap:12px;align-items:center;margin:4px 0 8px}.sortNote{color:#65728a;font-size:11px}.historyMissing{color:var(--muted);font-size:11px;text-align:center;padding:10px}
...
function actualHistory(r,n){let rows=A(n===5?r.last5:r.last10);if(rows.length)return rows.slice(0,n).map(x=>({value:Number(x.value),opponent_abbr:S(x.opponent_abbr,'-'),game_date:x.game_date||''}));let vals=A(n===5?r.last5_values:r.last10_values);return vals.slice(0,n).map(v=>({value:Number(v),opponent_abbr:'-',game_date:''}))}
function historyValues(r,n){return actualHistory(r,n).map(x=>x.value).filter(Number.isFinite)}
function historyHit(r,n,line,side){let vals=historyValues(r,n);if(!vals.length)return {h:0,p:0,n:0};let h=vals.filter(v=>isHit(v,line,side)).length;return {h,p:Math.round(h/vals.length*100),n:vals.length}}
function setPropSort(key){if(propSort.key===key)propSort.dir*=-1;else propSort={key,dir:1};drawProps()}
function sortValue(r,key){if(key==='player'||key==='stat')return String(r[key]||'').toLowerCase();if(key==='line')return Number(r.line??r.consensus_line??-999);if(key==='over')return Number(r.best_over_price??r.over_price??-9999);if(key==='under')return Number(r.best_under_price??r.under_price??-9999);let side=S(r.signal||r.side,'OVER'),line=Number(r.line??r.consensus_line??0);if(key==='l5')return historyHit(r,5,line,side).p;if(key==='l10')return historyHit(r,10,line,side).p;if(key==='match')return Number(r.confidence||r.final_score||0);return 0}
...
'''
    ...
    new_row = r'''function propRow(r){let team=teamFor(r),side=S(r.signal||r.side,'OVER'),line=Number(S(r.line||r.consensus_line,0)),hist5=actualHistory(r,5),hist10=actualHistory(r,10),v5=hist5.map(x=>x.value),v10=hist10.map(x=>x.value),h5=historyHit(r,5,line,side),h10=historyHit(r,10,line,side),conf=Number(r.confidence||r.final_score||0),valueClass=conf>=80?'value-high':conf>=65?'value-medium':'value-low',ob=r.best_over_book||r.book||'',ub=r.best_under_book||r.book||'',avg5=v5.length?(v5.reduce((a,b)=>a+b,0)/v5.length).toFixed(1):'-';return `<div class="propRow ${valueClass}"><div class="player"><div class="logo mono">${r.team_logo?`<img src="${E(r.team_logo)}" alt="${E(abbr(team))}">`:E(abbr(team).slice(0,2)||String(r.player||'?').slice(0,2))}</div><div><div class="name">${E(r.player)}</div><div class="team mono">${E(r.team_abbr||abbr(team))}</div></div></div><div class="stat mono">${E(r.stat)}</div><div class="lineVal mono">${E(S(r.line||r.consensus_line))}</div><div class="odds mono">${priceHtml(ob,r.best_over_price||r.over_price)}</div><div class="odds mono">${priceHtml(ub,r.best_under_price||r.under_price)}</div><div class="hist">${hist5.length?`<div class="boxes">${hist5.map(x=>`<div class="box ${isHit(x.value,line,side)?'':'miss'}" title="${E(x.game_date)} vs ${E(x.opponent_abbr)}"><div class="num mono">${E(x.value)}</div><div class="opp mono">${E(x.opponent_abbr)}</div></div>`).join('')}</div><div class="avg mono">L5 ${E(r.stat)} avg ${avg5}</div>`:'<div class="historyMissing mono">No completed game history</div>'}</div><div class="hit ${h5.n&&h5.p<50?'bad':''} mono"><div class="pct">${h5.n?h5.p+'%':'-'}</div><div>${h5.n?E(side):''}</div><div class="rec">${h5.n?h5.h+'/'+h5.n:''}</div></div><div class="hit ${h10.n&&h10.p<50?'bad':''} mono"><div class="pct">${h10.n?h10.p+'%':'-'}</div><div>${h10.n?E(side):''}</div><div class="rec">${h10.n?h10.h+'/'+h10.n:''}</div></div><div class="small mono match">${E(S(conf||'-'))}</div></div>`}
function props()'''
    ...
    print("Dashboard V4 Player Props polish applied with actual game history")
