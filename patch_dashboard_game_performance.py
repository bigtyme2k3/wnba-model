...
window.fullGamePerformance=function(){const s=D.summary||{},sp=D.spread||{},to=D.total||{},rows=Array.isArray(D.recent_games)?D.recent_games:[];const cards=rows.map(r=>`<article class="gp-card"><div class="gp-date mono">${esc(r.target_date||'')}</div><h3 class="mono">${esc(r.game||'Unknown game')}</h3><div class="gp-grid"><div><span>Spread pick</span><b>${esc(r.spread_recommendation||r.spread_pick||'PASS')}</b><em class="${String(r.spread_result||'').toLowerCase()}">${esc(r.spread_result||'PENDING')}</em></div><div><span>Total pick</span><b>${esc(r.total_recommendation||r.total_pick||'PASS')}</b><em class="${String(r.total_result||'').toLowerCase()}">${esc(r.total_result||'PENDING')}</em></div><div><span>Projected</span><b>${esc(r.projected_away_score)}–${esc(r.projected_home_score)}</b><small>Total ${esc(r.projected_total)}</small></div><div><span>Actual</span><b>${esc(r.actual_away_score)}–${esc(r.actual_home_score)}</b><small>Total ${esc(r.actual_total)}</small></div></div></article>`).join('');return `<div class="section"><h2 class="mono">Game Performance</h2><p class="small mono">Frozen pregame spreads, totals and projected scores graded against final results. Historical grading is evaluation data; model parameters are not auto-changed from this page.</p><div class="gp-summary"><div class="gp-metric"><span>Archived games</span><b>${Number(s.archived_games||0)}</b></div><div class="gp-metric"><span>Graded games</span><b>${Number(s.graded_games||0)}</b></div><div class="gp-metric"><span>Spread record</span><b>${rec(sp)}</b><small>${pct(sp.hit_rate)}</small></div><div class="gp-metric"><span>Total record</span><b>${rec(to)}</b><small>${pct(to.hit_rate)}</small></div><div class="gp-metric"><span>Margin MAE</span><b>${esc(s.avg_margin_error)}</b></div><div class="gp-metric"><span>Total MAE</span><b>${esc(s.avg_total_error)}</b></div></div><div class="gp-archive"><div class="gp-archive-head"><h3 class="mono">Graded Game Archive</h3><span class="small mono">${rows.length} recent graded games · scroll inside</span></div><div class="gp-scroll"><div class="gp-cards">${cards||'<div class="gp-card">No graded game cards yet.</div>'}</div></div></div></div>`};
    ...
    print({'status':'PASS','renderer':'fullGamePerformance','standalone_nav':False,'archive_scroll':True})
