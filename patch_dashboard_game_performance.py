...
window.fullGamePerformance=function(){
 const s=D.summary||{},sp=D.spread||{},to=D.total||{},rows=Array.isArray(D.recent_games)?D.recent_games:[],legacy=D.legacy_reference||{},ls=legacy.summary||{},lsp=legacy.spread||{},lto=legacy.total||{};
 ...
 return `<div class="section"><h2 class="mono">Game Performance</h2><p class="small mono">Current model only: ${esc(D.current_model_version||'unversioned')}. Legacy results are excluded from every current calibration number.</p>
 ...
 <div class="gp-panels"><div class="gp-panel"><h3 class="mono">Spread Forecast Calibration</h3><p class="small mono">${esc(sp.calibration?.diagnosis||'COLLECTING')} · Brier ${fixed(sp.calibration?.brier_score,4)} · ECE ${pct(sp.calibration?.expected_calibration_error)} · n=${Number(sp.calibration?.samples||0)}</p><table class="gp-table"><thead><tr><th>Raw band</th><th>N</th><th>Avg raw</th><th>Hit</th><th>Shrunk</th><th>95% CI</th><th>Status</th></tr></thead><tbody>${calRows(sp.calibration)||'<tr><td colspan="7">No graded current-model probability data.</td></tr>'}</tbody></table></div><div class="gp-panel"><h3 class="mono">Total Forecast Calibration</h3><p class="small mono">${esc(to.calibration?.diagnosis||'COLLECTING')} · Brier ${fixed(to.calibration?.brier_score,4)} · ECE ${pct(to.calibration?.expected_calibration_error)} · n=${Number(to.calibration?.samples||0)}</p><table class="gp-table"><thead><tr><th>Raw band</th><th>N</th><th>Avg raw</th><th>Hit</th><th>Shrunk</th><th>95% CI</th><th>Status</th></tr></thead><tbody>${calRows(to.calibration)||'<tr><td colspan="7">No graded current-model probability data.</td></tr>'}</tbody></table></div><div class="gp-panel"><h3 class="mono">Spread Edge Research</h3><table class="gp-table"><thead><tr><th>|Edge|</th><th>N</th><th>Record</th><th>Hit</th><th>Status</th></tr></thead><tbody>${edgeRows(sp.edge_buckets)||'<tr><td colspan="5">No graded current-model edge buckets.</td></tr>'}</tbody></table></div><div class="gp-panel"><h3 class="mono">Total Edge Research</h3><table class="gp-table"><thead><tr><th>|Edge|</th><th>N</th><th>Record</th><th>Hit</th><th>Status</th></tr></thead><tbody>${edgeRows(to.edge_buckets)||'<tr><td colspan="5">No graded current-model edge buckets.</td></tr>'}</tbody></table></div></div>
 <div class="gp-legacy"><h3 class="mono">${esc(legacy.label||'Legacy model — historical reference only')}</h3><p class="small mono">Excluded from current recommendations and calibration. Versions: ${esc((legacy.model_versions||[]).join(', '))}</p><div class="gp-legacy-grid"><div><span>Graded games</span><b>${Number(ls.graded_games||0)}</b></div><div><span>Winner accuracy</span><b>${pct(ls.winner_accuracy)}</b></div><div><span>Spread record</span><b>${rec(lsp)}</b></div><div><span>Total record</span><b>${rec(lto)}</b></div><div><span>Total signed error</span><b>${fixed(ls.total_bias)} pts</b></div><div><span>Total MAE</span><b>${fixed(ls.avg_total_error)} pts</b></div></div></div>
