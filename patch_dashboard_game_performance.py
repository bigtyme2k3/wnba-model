...
const fixed=(v,d=2)=>{const n=Number(v);return Number.isFinite(n)?n.toFixed(d):'—'};
...
const calRows=c=>(c?.buckets||[]).map(b=>`<tr><td>${esc(b.label)}</td><td>${Number(b.samples||0)}</td><td>${pct(b.avg_probability)}</td><td>${pct(b.hit_rate)}</td><td class="${Number(b.calibration_gap)>=0?'gp-pos':'gp-neg'}">${pct(b.calibration_gap)}</td></tr>`).join('');
const edgeRows=a=>(a||[]).map(b=>`<tr><td>${esc(b.label)}</td><td>${Number(b.samples||0)}</td><td>${Number(b.wins||0)}-${Number(b.losses||0)}-${Number(b.pushes||0)}</td><td>${pct(b.hit_rate)}</td></tr>`).join('');
window.fullGamePerformance=function(){
 ...
 return `<div class="section"><h2 class="mono">Game Performance</h2><p class="small mono">Frozen pregame predictions graded against final results. PASS is never counted as a betting win, and this page does not auto-change model parameters.</p>
 <div class="gp-summary"><div class="gp-metric"><span>Archive coverage</span><b>${Number(s.graded_games||0)}/${Number(s.archived_games||0)}</b><small>${pct(s.grade_coverage)} graded</small></div><div class="gp-metric"><span>Winner accuracy</span><b>${pct(s.winner_accuracy)}</b><small>${Number(s.winner_correct||0)}/${Number(s.winner_decisions||0)}</small></div><div class="gp-metric"><span>Spread ATS</span><b>${rec(sp)}</b><small>${pct(sp.hit_rate)}</small></div><div class="gp-metric"><span>Totals</span><b>${rec(to)}</b><small>${pct(to.hit_rate)}</small></div><div class="gp-metric"><span>Margin MAE</span><b>${fixed(s.avg_margin_error)}</b><small>${bias(s.margin_bias,'pts')}</small></div><div class="gp-metric"><span>Total MAE</span><b>${fixed(s.avg_total_error)}</b><small>${bias(s.total_bias,'pts')}</small></div><div class="gp-metric"><span>Away score MAE</span><b>${fixed(s.away_score_mae)}</b></div><div class="gp-metric"><span>Home score MAE</span><b>${fixed(s.home_score_mae)}</b></div></div>
 <div class="gp-panels"><div class="gp-panel"><h3 class="mono">Spread Calibration</h3><p class="small mono">Brier ${fixed(sp.calibration?.brier_score,4)} · ${Number(sp.calibration?.samples||0)} graded probability forecasts</p><table class="gp-table"><thead><tr><th>Prob band</th><th>N</th><th>Avg prob</th><th>Hit</th><th>Gap</th></tr></thead><tbody>${calRows(sp.calibration)||'<tr><td colspan="5">Not enough graded probability data.</td></tr>'}</tbody></table></div><div class="gp-panel"><h3 class="mono">Total Calibration</h3><p class="small mono">Brier ${fixed(to.calibration?.brier_score,4)} · ${Number(to.calibration?.samples||0)} graded probability forecasts</p><table class="gp-table"><thead><tr><th>Prob band</th><th>N</th><th>Avg prob</th><th>Hit</th><th>Gap</th></tr></thead><tbody>${calRows(to.calibration)||'<tr><td colspan="5">Not enough graded probability data.</td></tr>'}</tbody></table></div><div class="gp-panel"><h3 class="mono">Spread Edge Performance</h3><table class="gp-table"><thead><tr><th>|Edge|</th><th>N</th><th>Record</th><th>Hit</th></tr></thead><tbody>${edgeRows(sp.edge_buckets)||'<tr><td colspan="4">No graded edge buckets.</td></tr>'}</tbody></table></div><div class="gp-panel"><h3 class="mono">Total Edge Performance</h3><table class="gp-table"><thead><tr><th>|Edge|</th><th>N</th><th>Record</th><th>Hit</th></tr></thead><tbody>${edgeRows(to.edge_buckets)||'<tr><td colspan="4">No graded edge buckets.</td></tr>'}</tbody></table></div></div>
