    ...
    try:
        if p.suffix == ".json":
            ...
            if isinstance(data, dict):
                for key in ("markets", "props", "games", "players", "decisions", "best_bets", "rows", "top_decisions", "recommended_card"):
                    value = data.get(key)
                    if isinstance(value, list):
                        return len(value)
                summary = data.get("summary", {}) if isinstance(data.get("summary"), dict) else {}
                for key in ("markets", "props", "players", "games", "rows", "decision_rows", "evaluated_rows"):
...
def output_qa_modules(code: str) -> set[str]:
    mapping = {
        "DATE_MISMATCH": {"M02", "M20"},
        "BAD_PROBABILITY": {"M09", "M10", "M11", "M12", "M13"},
        "BAD_SCORE": {"M09", "M10", "M11", "M12", "M13", "M19"},
        "IMPLIED_PROBABILITY_MISMATCH": {"M03", "M04", "M13"},
        "UNSUPPORTED_BOOK": {"M03", "M04"},
        "UNSUPPORTED_MARKET": {"M10", "M13"},
        "INELIGIBLE_BET": {"M13", "M14", "M15"},
        "BET_WITH_FAILURES": {"M13", "M14", "M15"},
        "ELIGIBLE_NOT_BET": {"M13", "M14"},
        "BET_EV_OUT_OF_RANGE": {"M13"},
        "BET_PROBABILITY_TOO_LOW": {"M09", "M13"},
        "BET_HISTORY_TOO_LOW": {"M05", "M10", "M13"},
        "BET_BOOK_COUNT_TOO_LOW": {"M03", "M04", "M13"},
        "DECISION_COUNT_MISMATCH": {"M13", "M20"},
        "BET_COUNT_MISMATCH": {"M13", "M14", "M20"},
        "DUPLICATE_DECISION": {"M13"},
        "PORTFOLIO_CAP_EXCEEDED": {"M14", "M15"},
        "PORTFOLIO_PLAYER_DUPLICATION": {"M14", "M15"},
        "PORTFOLIO_GAME_CONCENTRATION": {"M14", "M15"},
        "STAKE_EXCEEDS_BANKROLL": {"M15"},
        "PORTFOLIO_SILENT_FAILURE": {"M14"},
        "PORTFOLIO_WITHOUT_QUALIFIED_BETS": {"M13", "M14"},
    }
...
def main() -> None:
    ...
    completion = round(ready / max(len(modules), 1) * 100, 1)
    ...
    out = {
        ...
        "qa": {
            ...
            "repository_qa_status": repo_qa.get("status", "unknown"),
            "output_qa_status": output_qa.get("status", "unknown"),
            "repository_qa_generated_at": repo_qa.get("generated_at_utc"),
            "output_qa_generated_at": output_qa.get("generated_at_utc"),
            "output_summary": output_qa.get("summary", {}),
        },
        "summary": {
            "modules": len(modules),
            ...
            "release_blockers": sum(len(m.get("blockers", [])) for m in modules),
            "warnings": sum(len(m.get("warnings", [])) for m in modules),
        },
        "modules": modules,
        "release_blockers": [{"id": m["id"], "module": m["name"], "items": m["blockers"]} for m in modules if m["blockers"]],
